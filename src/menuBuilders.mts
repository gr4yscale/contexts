...
import {
  menuEmacsWindowBookmarks,
  menuEmacsOrgBookmarks,
  ...
  await buildMenu({
    ...
    builder: () => [
      ...
      { display: "emacs capture" /*handler: () => emacsCapture() */ },
      {
        ...
        builder: () => menuEmacsWindowBookmarks(),
      },
      {
        display: "emacs org bookmarks",
        builder: () => menuEmacsOrgBookmarks(),
      },
      { display: "links by group", builder: () => menuLinkGroups() },
...
export const buildMenuLaunchItems = async () => {
  const c = await getCurrentActivity();
  if (!c) {
    return;
  }
  //const linksAll = linkGroups.flatMap((lg) => lg.links);

  const menuItemLinks = c.links
